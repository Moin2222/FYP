...
def Create_user_type(user_type: User_Type):
    try:
        ...
        # Check if the user_type already exists to avoid duplicate entries
        cursor.execute("SELECT 1 FROM User_Type WHERE user_type = ?", (user_type.user_type,))
        if cursor.fetchone():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User type already exists")
...
# Endpoint to activate a profile 
